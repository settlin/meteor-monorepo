...
export const Migration = {
   ...
   * @static
   */
	_isNewRole: role => !role.name && role.children,
   ...
   * @static
   */
	_isOldRole: role => role.name && !role.children,
   ...
   * @static
   */
	_isNewField: roles => Array.isArray(roles) && roles[0] === Object(roles[0]),
   ...
   * @static
   */
	_isOldField: roles => (Array.isArray(roles) && typeof roles[0] === 'string') || (!Array.isArray(roles) && roles === Object(roles)),
   ...
   */
	_convertToNewField: function(oldRoles, convertUnderscoresToDots) {
		...
		else if (typeof oldRoles === 'object' && oldRoles) {
			Object.keys(oldRoles).forEach(function(group) {
				const rolesArray = oldRoles[group];
   ...
   */
	_convertToOldField: function(newRoles, usingGroups) {
		...
		newRoles.forEach(function(userRole) {
			if (userRole !== Object(userRole)) throw new Error("Role '" + userRole + "' is not an object.");
   ...
   * @static
   */
	_forwardMigrate: function(updateUser, updateRole, convertUnderscoresToDots, query) {
		updateUser = updateUser || Migration._defaultUpdateUser;
		updateRole = updateRole || Migration._defaultUpdateRole;

		Migration._dropCollectionIndex(Meteor.roles, 'name_1');

		Meteor.roles.find(query).forEach(role => !Migration._isNewRole(role) && updateRole(role, Migration._convertToNewRole(role)));
		Meteor.users.find(query).forEach(user => !Migration._isNewField(user.roles) && updateUser(user, Migration._convertToNewField(user.roles, convertUnderscoresToDots)));
	},
   ...
   */
	_backwardMigrate: function(updateUser, updateRole, usingGroups) {
		...
		Migration._dropCollectionIndex(Meteor.users, 'roles._id_1_roles.scope_1');
		Migration._dropCollectionIndex(Meteor.users, 'roles.scope_1');

		Meteor.roles.find().forEach(role => !Migration._isOldRole(role) && updateRole(role, Migration._convertToOldRole(role)));
		Meteor.users.find().forEach(user => !Migration._isOldField(user.roles) && updateUser(user, Migration._convertToOldField(user.roles, usingGroups)));
	},
};
