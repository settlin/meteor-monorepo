...
(function() {
	...
	function _innerTest(test, userParam, username, expectedRoles, scope) {
		...
		roles.forEach(function(role) {
			var expected = !!~expectedRoles.indexOf(role),
	...
	function itemsEqual(test, actual, expected) {
		...
		function intersectionObjects(/*args*/) {
			var array, rest;
			array = arguments[0];
			rest = arguments.length >= 2 ? _.toArray(arguments).slice(1) : [];
			return _.uniq(array).filter(function(item) {
	...
	Tinytest.add(
		...
		function(test) {
			...
			test.equal(typeof Meteor.users.findOne({_id: '1'}), 'undefined');
		});
	...
	Tinytest.add(
		...
		function(test) {
			...
			test.isFalse(typeof Roles.userIsInRole(users.eve, 'undefined'));
			...
			test.isFalse(typeof Roles.userIsInRole(users.eve, 'undefined', {anyScope: true}));
		});
}());
