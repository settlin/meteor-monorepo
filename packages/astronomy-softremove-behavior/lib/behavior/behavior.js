...
Behavior.create({
  ...
  createClassDefinition() {
    ...
    const definition = {
      fields: {},
      events: {
        beforeFind: beforeFind.bind(behavior)
      },
  ...
  apply(Class) {
    ...
    if (connection) {
      ...
      for (const key in meteorMethods) {
        if (meteorMethods.hasOwnProperty(key)) {
          const methodName = meteorMethods[key];
          // Add meteor method.
          if (!hasMeteorMethod(connection, methodName)) connection.methods({[methodName]: meteorMethods[key]});
        }
      }
    }
